...
class TestHooks extends PHPUnit_Framework_TestCase
{
	...
	public function testAddHooks()
	{
		...
		{
			$GLOBALS['elk']['hooks']->add($this->_hook_name, $test['call'], $test['file'], $test['perm']);
	...
	{
		$this->_call = $GLOBALS['elk']['hooks']->hook($this->_hook_name);
	...
	public function testRemoveHooks()
	{
		foreach ($this->_tests as $test)
		{
			$GLOBALS['elk']['hooks']->remove($this->_hook_name, $test['call'], $test['file']);
	 ...
	 */
	protected function _hook_exists($expected_hook, $permanent)
	{
		global $modSettings;
		...
		list ($db_hook_string) = $request->fetchRow();
		$request->free();
	 ...
	 * @return string[]
	 */
	protected function _parse_hooks($hook_string)
	{
		$hooks = array();
	 ...
	 * @return bool
	 */
	protected function _find_hook($expected_hook, $hooks)
	{
	 ...
	 * @return bool
	 */
	protected function _is_hook_called($hook_string)
	{
