...
class TestUtilclass extends PHPUnit_Framework_TestCase
{
	...
	{
		$string = '8237'; // 0x202D
		$this->assertEmpty($GLOBALS['elk']['text']->entity_fix($string));
	}
	...
	public function test_htmlspecialchars()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->htmlspecialchars($string), $actual1);

		$modSettings['disableEntityCheck'] = true;
		$this->assertEquals($GLOBALS['elk']['text']->htmlspecialchars($string, ENT_COMPAT, 'UTF-8', true), $actual2);
	}
	...
	public function test_htmltrim()
	{
		...
		$GLOBALS['elk']['text']->htmltrim($string);
		$this->assertEquals($GLOBALS['elk']['text']->htmltrim($string), $actual1);
	}
	...
	public function test_strpos()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->strpos($this->string, 'elkarte'), 30);
	}
	...
	{
		// Should be 30, strpos would say 39
		$this->assertEquals($GLOBALS['elk']['text']->substr($this->string, 30, 7), 'elkarte');
	}
	...
	public function test_uppercase()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->strtoupper($string), $actual);
	}
	...
	public function test_lowercase()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->strtolower($string), $actual);
	}
	...
	public function test_shorten_text()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->shorten_text($this->string, 26, true, ''), $actual);
	}
	...
	public function test_shorten_html()
	{
		...
		$this->assertEquals($GLOBALS['elk']['text']->shorten_html($string, 12), $actual);
	}
	...
	{
		$this->assertEquals($GLOBALS['elk']['text']->strlen($this->string), 45);
	}
}