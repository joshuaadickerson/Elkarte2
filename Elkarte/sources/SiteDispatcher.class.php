...
class Site_Dispatcher
{
	...
	public function __construct()
	{
		...
		Hooks::get()->hook('action_frontpage', array(&$this->_default_action));
		...
		Hooks::get()->hook('actions', array(&$actionArray, &$adminActions));
	...
	public function dispatch()
	{
		if (!empty($this->_controller_name))
		{
			...
			Hooks::get()->hook('action_' . $hook . '_before', array($this->_function_name));

			$result = $controller->$method();

			Hooks::get()->hook('action_' . $hook . '_after', array($this->_function_name));
