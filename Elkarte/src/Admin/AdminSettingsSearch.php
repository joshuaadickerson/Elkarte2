...
class AdminSettings_Search
{
	...
	protected function _find_term($search_term, $item)
	{
		...
		if ($found !== false)
		{
			...
			$return = array(
				'name' => $name,
				'help' => $GLOBALS['elk']['text']->shorten_text(isset($item[2]) ? strip_tags($helptxt[$item[2]]) : (isset($helptxt[$found]) ? strip_tags($helptxt[$found]) : ''), 255),
			);
		}
	 ...
	 * @todo move to subs?
	 */
	protected function _load_settings($settings_search)
	{
		global $elk;
	 ...
	 */
	protected function _load_search_sections()
	{
		global $context;
