...
class InstallController
{
    ...
    protected function action_welcome()
    {
        ...
        foreach ($databases as $key => $db)
        {
            if ($db['supported'])
            {
                if (!empty($db['additional_file']) && !file_exists(__DIR__ . 'InstallController.php/' . $db['additional_file']))
                {
    ...
    protected function action_databasePopulation()
    {
        ...
        {
            parse_sqlLines(__DIR__ . 'InstallController.php/' . $databases[$db_type]['additional_file'], $replaces);
        }
