...
class MaintenanceController extends AbstractController
{
	...
	public function action_index()
	{
		...
		$this->_templates->load('Maintenance');
	...
	public function action_routine()
	{
		...
		$GLOBALS['elk']['hooks']->hook('routine_maintenance', array(&$context['routine_actions']));
	}
	...
	public function action_topics()
	{
		...
		$this->_templates->load('GenericBoards');
	...
	public function action_repair_display()
	{
		...
		$controller = new RepairBoardsController(new Event_manager());
	...
	public function action_optimize_display()
	{
		...
		if ($context['num_tables'] == 0)
			$GLOBALS['elk']['errors']->fatal_error('You appear to be running ElkArte in a flat file mode... fantastic!', false);
	 ...
	 */
	protected function _buildContinue($percent, $step)
	{
		global $context, $txt;
	...
	public function action_reattribute_display()
	{
		...
		if (($our_post['type'] === 'name' && !empty($our_post['from_name'])) || ($our_post['type'] === 'email' && !$validator->validation_errors('from_email')))
		{
			...
			if (empty($members))
				$GLOBALS['elk']['errors']->fatal_lang_error('reattribute_cannot_find_member');
	...
	public function action_massmove_display()
	{
		...
		$GLOBALS['elk']['cache']->remove('board-' . $id_board_from);
		$GLOBALS['elk']['cache']->remove('board-' . $id_board_to);
