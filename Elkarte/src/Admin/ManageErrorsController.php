 ...
 */
class ManageErrorsController extends AbstractController
{
	/**
	 ...
	 * @param array $filter
	 */
	protected function _applyFilter($filter)
	{
	 ...
	 * @param int[] $members
	 */
	protected function _loadMemData($members)
	{
	...
	protected function action_viewfile()
	{
		...
		if ($ext !== '.php' || (strpos($file, $real_board) === false && strpos($file, $real_source) === false) || strpos($file, $real_cache) !== false || in_array($basename, $excluded) || !is_readable($file))
			$GLOBALS['elk']['errors']->fatal_lang_error('error_bad_file', true, array(htmlspecialchars($filename, ENT_COMPAT, 'UTF-8')));
		...
		if ($max <= 0 || $min >= $max)
			$GLOBALS['elk']['errors']->fatal_lang_error('error_bad_line');
