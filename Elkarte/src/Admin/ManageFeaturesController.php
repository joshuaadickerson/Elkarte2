 ...
 */
class ManageFeaturesController extends AbstractController
{
	/**
	...
	public function action_layoutSettings_display()
	{
		...
		if (isset($this->http_req->query->save))
		{
			...
			{
				$GLOBALS['elk']['hooks']->remove('integrate_action_frontpage', $modSettings['front_page'] . '::frontPageHook');
			}
	 ...
	 */
	protected function _initMentionSettingsForm()
	{
		global $txt, $context;
	...
	public function action_profileedit()
	{
		...
		elseif (isset($this->http_req->post->save))
		{
			...
			if (empty($context['fid']))
			{
				...
				if (!$unique)
					$GLOBALS['elk']['errors']->fatal_lang_error('custom_option_not_unique');
			}
	 ...
	 */
	protected function _initPMSettingsForm()
	{
		global $context;
	 ...
	 */
	protected function _basicSettings()
	{
		...
		$config_vars = array(
			...
			'',
				// SEO stuff
				array('check', 'queryless_urls', 'subtext' => '<strong>' . $txt['queryless_urls_note'] . '</strong><br />' . (serverIs('apache') || serverIs('lighttpd') ? $txt['queryless_urls_work'] : '<span class="error">' . $txt['queryless_urls_notwork'] . '</span>')),
			'',
		...
		$GLOBALS['elk']['hooks']->hook('modify_basic_settings', array(&$config_vars));
	 ...
	 */
	protected function _layoutSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_layout_settings', array(&$config_vars));
	 ...
	 */
	protected function _karmaSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_karma_settings', array(&$config_vars));
	 ...
	 */
	protected function _likesSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_likes_settings', array(&$config_vars));
	 ...
	 */
	protected function _notificationsSettings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_mention_settings', array(&$config_vars));
	 ...
	 */
	protected function _signatureSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_signature_settings', array(&$config_vars));
	 ...
	 * - Used in Admin center search and settings form
	 */
	protected function _pmSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_pmsettings_settings', array(&$config_vars));
