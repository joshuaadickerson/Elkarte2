 ...
 * @package Languages
 */
class ManageLanguagesController extends AbstractController
{
	/**
	...
	public function action_downloadlang()
	{
		...
		if (!empty($this->_req->post->do_install) && !empty($this->_req->post->copy_file))
		{
			...
			foreach ($this->_req->post->copy_file as $file)
			{
				...
				if (strpos($file, '..') !== false || (strpos($file, 'themes') !== 0 && !preg_match('~agreement\.[A-Za-z-_0-9]+\.txt$~', $file)))
					$GLOBALS['elk']['errors']->fatal_error($txt['languages_download_illegal_paths']);
		...
		$GLOBALS['elk']['cache']->put('known_languages', null, $GLOBALS['elk']['cache']->isEnabled() && !$GLOBALS['elk']['cache']->checkLevel(1) ? 86400 : 3600);
	...
	public function action_editlang()
	{
		...
		$context['primary_settings'] = array(
			'name' => $GLOBALS['elk']['text']->ucwords(strtr($context['lang_id'], array('_' => ' ', '-utf8' => ''))),
	...
	protected function _settings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_language_settings', array(&$config_vars));
