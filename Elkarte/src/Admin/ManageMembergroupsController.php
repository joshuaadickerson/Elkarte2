 ...
 */
class ManageMembergroupsController extends AbstractController
{
	/**
	...
	*/
			...
			elseif ($this->http_req->post->perm_type == 'copy' || $this->http_req->post->perm_type == 'inherit')
			{
				...
				if (!allowedTo('admin_forum'))
				{
					...
					if ($copy_type['group_type'] == 1)
						$GLOBALS['elk']['errors']->fatal_lang_error('membergroup_does_not_exist');
				}
	...
	public function action_edit()
	{
		...
		elseif (isset($this->http_req->post->save))
		{
			...
			$GLOBALS['elk']['hooks']->hook('save_membergroup', array($current_group['id_group']));
		...
		$GLOBALS['elk']['hooks']->hook('view_membergroup');
	...
	protected function _settings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_membergroup_settings', array(&$config_vars));
