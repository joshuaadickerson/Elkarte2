 ...
 */
class ManageServerController extends AbstractController
{
	/**
	...
	public function action_cacheSettings_display()
	{
		...
		if ($cache_accelerator === 'filebased' && $GLOBALS['elk']['cache']->isEnabled() && extension_loaded('Zend OPcache'))
		{
		...
		$GLOBALS['elk']['layers']->add('clean_cache_button');
	...
	public function action_balancingSettings_display()
	{
		...
		$GLOBALS['elk']['hooks']->hook('loadavg_settings');
	 ...
	 */
	protected function _generalSettings()
	{
		global $txt;
		...
		$GLOBALS['elk']['hooks']->hook('modify_general_settings', array(&$config_vars));
	 ...
	 */
	protected function _databaseSettings()
	{
		global $txt;
		...
		// Notify the integration
		$GLOBALS['elk']['hooks']->hook('modify_database_settings', array(&$config_vars));
	...
	protected function _cookieSettings()
	{
		...
		// Notify the integration
		$GLOBALS['elk']['hooks']->hook('modify_cookie_settings', array(&$config_vars));
	...
	protected function _cacheSettings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_cache_settings', array(&$config_vars));
	 ...
	 */
	protected function _balancingSettings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_loadavg_settings', array(&$config_vars));
