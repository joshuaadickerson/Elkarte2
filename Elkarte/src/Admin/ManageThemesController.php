 ...
 * @package Themes
 */
class ManageThemesController extends AbstractController
{
	/**
	...
	public function action_options()
	{
		...
		if (isset($this->http_req->post->submit) && empty($this->http_req->post->who))
		{
			...
			$GLOBALS['elk']['cache']->remove('theme_settings-' . $theme);
			$GLOBALS['elk']['cache']->remove('theme_settings-1');
		...
		$this->_templates->load('Settings');
		$this->_templates->loadSubTemplate('options');
	...
	public function action_setthemesettings()
	{
		...
		if (empty($theme))
			$GLOBALS['elk']['errors']->fatal_lang_error('no_theme', false);
		...
		// Let the theme take care of the settings.
		$this->_templates->load('Settings');
		$this->_templates->loadSubTemplate('settings');
		...
		if (file_exists($settings['theme_dir'] . '/index.template.php'))
		{
			...
			if (preg_match('~\'theme_variants\'\s*=>(.+?\)),$~sm', $file_contents, $matches))
				...
				$GLOBALS['elk']['hooks']->hook('init_theme', array($theme, &$settings));
		}
		...
		foreach ($settings as $setting => $dummy)
		{
			if (!in_array($setting, array('theme_url', 'theme_dir', 'images_url', 'template_dirs')))
				$settings[$setting] = $GLOBALS['elk']['text']->htmlspecialchars__recursive($settings[$setting]);
		}
		...
		if (isset($this->http_req->query->th))
		{
			...
			if (!isset($u) || !allowedTo('admin_forum'))
			{
				...
				if (!empty($vrt))
				{
					...
					$GLOBALS['elk']['cache']->remove('theme_settings-' . $th . ':' . $user_info['id']);
			...
			if (!empty($vrt) && ($u === 0 || $u === -1))
			{
				...
				$GLOBALS['elk']['cache']->remove('theme_settings-' . $th);
			}
			...
			else
			{
				...
				if (!empty($vrt))
				{
					...
					$GLOBALS['elk']['cache']->remove('theme_settings-' . $th . ':' . $u);
	...
	public function action_install()
	{
		...
		elseif ($method == 'upload')
			...
			$GLOBALS['elk']['errors']->fatal_lang_error('theme_install_general', false);
	 ...
	 */
	protected function _action_edit_style()
	{
	 ...
	 * ?action=Admin;area=theme;sa=edit
	 */
	protected function _action_edit_template()
	{
		global $context;
	 ...
	 * ?action=Admin;area=theme;sa=edit
	 */
	protected function _action_edit_file()
	{
		global $context;
	 ...
	 * ?action=Admin;area=theme;sa=edit
	 */
	protected function _action_edit_submit()
	{
		...
		if (empty($selectedTheme))
		{
			...
			$GLOBALS['elk']['errors']->fatal_lang_error('theme_edit_missing');
		}
	 ...
	 */
	protected function prepareThemeEditContext()
	{
		global $context;
