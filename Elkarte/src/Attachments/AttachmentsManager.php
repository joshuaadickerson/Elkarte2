...
class AttachmentsManager
{
	...
	function approveAttachments($attachments)
	{
		...
		$db->fetchQueryCallback('
			...
			function ($row) {
				logAction(
					...
					array(
						'message' => $row['id_msg'],
						'filename' => preg_replace('~&amp;#(\\d{1,7}|x[0-9a-fA-F]{1,6});~', '&#\\1;', $GLOBALS['elk']['text']->htmlspecialchars($row['filename'])),
					)
				);
			}
		);
		...
		$GLOBALS['elk']['hooks']->hook('approve_attachments', array($attachments));
	}
	...
	function removeAttachments($condition, $query_type = '', $return_affected_messages = false, $autoThumbRemoval = true)
	{
		...
		$GLOBALS['elk']['hooks']->hook('remove_attachments', array($attach));
	...
	function getAttachmentCount()
	{
		...
		list ($num_attachments) = $request->fetchRow();
		$request->free();
	...
	function getAvatarCount()
	{
		...
		list ($num_avatars) = $request->fetchRow();
		$request->free();
	...
	function attachment_folder($attach_id, $folder_id = null)
	{
		...
		if ($folder_id === null) {
			...
			if (!empty($result)) {
				list ($folder_id) = $result->fetchRow();
				$result->free();
				return $folder_id;
			}
			return false;
	...
	function maxNoThumb()
	{
		...
		list ($thumbnails) = $result->fetchRow();
		$result->free();
	...
	function getMaxThumbnail()
	{
		...
		list ($thumbnail) = $result->fetchRow();
		$result->free();
	...
	function maxAttachment()
	{
		...
		list ($attachment) = $result->fetchRow();
		$result->free();
	...
	function list_getNumUnapprovedAttachments($approve_query)
	{
		...
		list ($total_unapproved_attachments) = $request->fetchRow();
		$request->free();
	...
	function list_getNumFiles($browse_type)
	{
		...
		list ($num_files) = $request->fetchRow();
		$request->free();
	...
	function overallAttachmentsSize()
	{
		...
		list ($attachmentDirSize) = $request->fetchRow();
		$request->free();
	...
	function attachDirProperties($dir)
	{
		...
		list ($current_dir['files'], $current_dir['size']) = $request->fetchRow();
		$request->free();
	...
	function countAttachmentsInFolders($id_folder)
	{
		...
		list ($num_attach) = $request->fetchRow();
		$request->free();
