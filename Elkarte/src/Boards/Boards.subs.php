...
function modifyBoard($board_id, &$boardOptions)
{
	...
	if (isset($boardOptions['move_to']))
	{
		...
		elseif ($boardOptions['move_to'] == 'child')
		{
			...
			if (isChildOf($id_parent, $board_id))
				$GLOBALS['elk']['errors']->fatal_lang_error('mboards_parent_own_child_error', false);
			elseif ($id_parent == $board_id)
				$GLOBALS['elk']['errors']->fatal_lang_error('mboards_board_own_child_error', false);
	...
	$GLOBALS['elk']['hooks']->hook('modify_board', array($board_id, $boardOptions, &$boardUpdates, &$boardUpdateParameters));
	...
	if (isset($boardOptions['moderators']) || isset($boardOptions['moderator_string']))
	{
		...
		if (isset($boardOptions['moderator_string']) && trim($boardOptions['moderator_string']) != '')
		{
			...
			$moderator_string = strtr($GLOBALS['elk']['text']->htmlspecialchars($boardOptions['moderator_string'], ENT_QUOTES), array('&quot;' => '"'));
...
function deleteBoards($boards_to_remove, $moveChildrenTo = null)
{
	...
	$GLOBALS['elk']['hooks']->hook('delete_board', array($boards_to_remove, &$moveChildrenTo));
		...
		if (!empty($row['id_board']))
		{
			...
			else
			{
				...
				if (!isset($boards[$row['id_parent']]['tree']))
					$GLOBALS['elk']['errors']->fatal_lang_error('no_valid_parent', false, array($row['board_name']));
...
function incrementBoard($id_board, $values)
{
	...
	$GLOBALS['elk']['hooks']->hook('board_fields', array(&$knownInts));
