...
class Categories
{
	...
	public function modify($category_id, $catOptions)
	{
		...
		$cat_id = $category_id;
		$this->hooks->hook('modify_category', array($cat_id, &$catUpdates, &$catParameters));
	...
	function create($catOptions)
	{
		...
		$this->hooks->hook('create_category', array(&$catOptions, &$cat_columns, &$cat_parameters));
		...
		$category_id = $result->insertId('{db_prefix}categories', 'id_cat');

		// Set the given properties to the newly created category.
		$this->modify($category_id, $catOptions);
	 ...
	 */
	function delete(array $categories, $moveBoardsTo = null)
	{
		global $cat_tree;
