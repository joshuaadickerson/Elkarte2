...
class RepairBoards
{
	...
	function createSalvageArea()
	{
		...
		if ($result->numRows() != 0)
			list ($salvageCatID) = $result->fetchRow();
		$result->free();
		...
		if (empty($salvageCatID)) {
			...
			$salvageCatID = $result->insertId('{db_prefix}categories', 'id_cat');
		}
		...
		if ($result->numRows() != 0)
			list ($salvageBoardID) = $result->fetchRow();
		$result->free();
		...
		if (empty($salvageBoardID)) {
			...
			$salvageBoardID = $result->insertId('{db_prefix}boards', 'id_board');
		}
