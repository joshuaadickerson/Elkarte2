...
function canLinkEvent()
{
	...
	// No board?  No topic?!?
	if (empty($board))
		$GLOBALS['elk']['errors']->fatal_lang_error('missing_board_id', false);
	if (empty($topic))
		$GLOBALS['elk']['errors']->fatal_lang_error('missing_topic_id', false);
	...
	if (!allowedTo('admin_forum') && !allowedTo('moderate_board'))
	{
		...
		if (!empty($row))
		{
			...
			if ($row['id_member_started'] != $user_info['id'])
				$GLOBALS['elk']['errors']->fatal_lang_error('not_your_topic', 'user');
		}
		// Topic/Board doesn't exist.....
		else
			$GLOBALS['elk']['errors']->fatal_lang_error('calendar_no_topic', 'general');
	}
}
...
function insertEvent(&$eventOptions)
{
	...
	$eventOptions['title'] = $GLOBALS['elk']['text']->htmlspecialchars($eventOptions['title'], ENT_QUOTES);
	...
	$GLOBALS['elk']['hooks']->hook('create_event', array(&$eventOptions, &$event_columns, &$event_parameters));
	...
	$eventOptions['id'] = $result->insertId('{db_prefix}calendar', 'id_event');
...
function modifyEvent($event_id, &$eventOptions)
{
	...
	$GLOBALS['elk']['hooks']->hook('modify_event', array($event_id, &$eventOptions, &$event_columns));
...
function list_getNumHolidays()
{
	...
	list ($num_items) = $request->fetchRow();
	$request->free();
