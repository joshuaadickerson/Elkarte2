...
class CalendarEvent
{
	...
	public function validate($event)
	{
		...
		if (isset($event['span']))
		{
			...
			if (empty($this->_settings['cal_allowspan']))
				throw new ElkException('no_span', false);
			if ($event['span'] < 1 || $event['span'] > $this->_settings['cal_maxspan'])
				throw new ElkException('invalid_days_numb', false);
		}
		...
		if (!isset($event['deleteevent']))
		{
			...
			if (!isset($event['month']))
				throw new ElkException('event_month_missing', false);
			if (!isset($event['year']))
				throw new ElkException('event_year_missing', false);

			// Check the month and year...
			if ($event['month'] < 1 || $event['month'] > 12)
				throw new ElkException('invalid_month', false);
			if ($event['year'] < $this->_settings['cal_minyear'] || $event['year'] > $this->_settings['cal_maxyear'])
				throw new ElkException('invalid_year', false);

			// No day?
			if (!isset($event['day']))
				throw new ElkException('event_day_missing', false);

			if (!isset($event['evtitle']) && !isset($event['subject']))
				throw new ElkException('event_title_missing', false);
			elseif (!isset($event['evtitle']))
				$event['evtitle'] = $event['subject'];

			// Bad day?
			if (!checkdate($event['month'], $event['day'], $event['year']))
				throw new ElkException('invalid_date', false);

			// No title?
			if ($GLOBALS['elk']['text']->htmltrim($event['evtitle']) === '')
				throw new ElkException('no_event_title', false);
			if ($GLOBALS['elk']['text']->strlen($event['evtitle']) > 100)
				$event['evtitle'] = $GLOBALS['elk']['text']->substr($event['evtitle'], 0, 100);
	...
	public function insert($options, $member_id)
	{
		$eventOptions = array(
			...
			'title' => $GLOBALS['elk']['text']->substr($options['evtitle'], 0, 100),
			'member' => $member_id,
	...
	public function remove()
	{
		...
		$GLOBALS['elk']['hooks']->hook('remove_event', array($this->_event_id));
