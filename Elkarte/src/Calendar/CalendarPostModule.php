...
class Calendar_Post_Module implements ElkArte\sources\modules\Module_Interface
{
	...
	{
		$GLOBALS['elk']['layers']->addAfter('make_event', 'postarea');
	}

	public function before_save_post($post_errors)
	{
		if (!isset($_REQUEST['deleteevent']) && $GLOBALS['elk']['text']->htmltrim($_POST['evtitle']) === '')
	...
	public function prepare_context($id_member_poster)
	{
		...
		if ($event_id != -1 && !isset($_REQUEST['subject']))
		{
			...
			{
				throw new ControllerRedirectException('CalendarController', 'action_post');
			}
		}
	 ...
	 */
	protected function _prepareEventContext($event_id)
	{
		...
		else
		{
			...
			if ($context['event']['month'] < 1 || $context['event']['month'] > 12)
				$GLOBALS['elk']['errors']->fatal_lang_error('invalid_month', false);
			...
			if (empty($boards))
				$GLOBALS['elk']['errors']->fatal_lang_error('cannot_post_new', 'user');
