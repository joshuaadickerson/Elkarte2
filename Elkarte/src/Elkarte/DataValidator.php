...
class DataValidator
{
	 ...
	 * @param mixed[] $ruleset
	 */
	protected function _validate($input, $ruleset)
	{
		// No errors ... yet ;)
	 ...
	 * @param string $rules
	 */
	protected function _validate_recursive($input, $field, $rules)
	{
	 ...
	 * @return mixed
	 */
	protected function _sanitize($input, $ruleset)
	{
		...
		foreach ($ruleset as $field => $rules)
		{
			...
			else
			{
				...
				foreach ($rules as $rule)
				{
					...
					// Were any parameters provided for the rule, e.g. $GLOBALS['elk']['text']->htmlspecialchars[ENT_QUOTES]
					if (preg_match('~(.*)\[(.*)\]~', $rule, $match))
					{
	 ...
	 * @param string $rules
	 */
	protected function _sanitize_recursive($input, $field, $rules)
	{
	 ...
	 */
	protected function _get_error_messages($keys)
	{
		global $txt;
	...
	protected function _validate_max_length($field, $input, $validation_parameters = null)
	{
		...
		if ($GLOBALS['elk']['text']->strlen($input[$field]) <= (int) $validation_parameters)
			return;
	...
	/**
	 * Uses $GLOBALS['elk']['text']->htmlspecialchars to sanitize any html in the intput
	 *
	 * @param string $input
	 * @param string|null $sanitation_parameters
	 */
	protected function _sanitation_cleanhtml($input, $sanitation_parameters = null)
	{
		...
		return $GLOBALS['elk']['text']->htmlspecialchars($input);
	}
}