...
class Request
{
	...
	protected function _checkExit()
	{
		...
		if (strpos($_SERVER['QUERY_STRING'], 'http') === 0)
		{
			...
			$GLOBALS['elk']['errors']->fatal_error('', false);
		}
	...
	protected function _checkNumericKeys()
	{
		...
		{
			if (is_numeric($key))
				$GLOBALS['elk']['errors']->fatal_error('Numeric request keys are invalid.', false);
		}
	...
	protected function _checkMagicQuotes()
	{
		if (version_compare(PHP_VERSION, '5.4.0', '<'))
		{
			...
			if (ini_get('magic_quotes_sybase') || strtolower(ini_get('magic_quotes_sybase')) == 'on')
				$GLOBALS['elk']['errors']->fatal_error('magic_quotes_sybase=on was detected: your host is using an unsecure PHP configuration, deprecated and removed in current versions. Please upgrade PHP.', false);

			// Reject magic_quotes_gpc='on'.
			if (function_exists('get_magic_quotes_gpc') && get_magic_quotes_gpc() != 0)
				$GLOBALS['elk']['errors']->fatal_error('magic_quotes_gpc=on was detected: your host is using an unsecure PHP configuration, deprecated and removed in current versions. Please upgrade PHP.', false);
		}
	}
