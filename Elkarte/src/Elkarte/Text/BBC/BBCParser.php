...
namespace Elkarte\Elkarte\Text\BBC;
...
class BBCParser
{
	...
	{
		$GLOBALS['elk']['hooks']->hook('pre_bbc_parser', array(&$message, $this->bbc));
		...
		$GLOBALS['elk']['hooks']->hook('post_bbc_parser', array(&$message));
		$this->message = $message;
	...
	public function loadHtmlParser()
	{
		...
		$GLOBALS['elk']['hooks']->hook('bbc_load_html_parser', array(&$parser));
	...
	protected function recursiveParser(&$data, array $tag)
	{
		...
		$autolinker = null;
		$html = null;
		$GLOBALS['elk']['hooks']->hook('recursive_bbc_parser', array(&$autolinker, &$html));
