...
class StringUtil
{
	protected $entity_check_reg = '~(&#(\d{1,7}|x[0-9a-fA-F]{1,6});)~';
	protected $entity_check_regex_html = '~(&amp;#(\d{1,7}|x[0-9a-fA-F]{1,6});)~';
	protected $disable_entity_check = false;

	public function __construct($disable_entity_check)
	{
		// $modSettings['disableEntityCheck']
		$this->disable_entity_check = (bool) $disable_entity_check;
	}
	 ...
	 * @return string
	 */
	public function entity_fix($string)
	{
	 ...
	 * @return string
	 */
	public function htmlspecialchars($string, $quote_style = ENT_COMPAT, $charset = 'UTF-8', $double = false)
	{
		if (empty($string))
			return $string;
	...
	public function htmltrim($string)
	{
		...
		if (!$this->disable_entity_check)
			$check = preg_replace('~^(?:[ \t\n\r\x0B\x00' . $space_chars . ']|&nbsp;)+|(?:[ \t\n\r\x0B\x00]|&nbsp;)+$~u', '', preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $string));
		else
			$check = preg_replace('~^(?:[ \t\n\r\x0B\x00' . $space_chars . ']|&nbsp;)+|(?:[ \t\n\r\x0B\x00]|&nbsp;)+$~u', '', $string);
	...
	{
		$haystack_check = !$this->disable_entity_check ? preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $haystack) : $haystack;
		...
		{
			$needle_check = !$this->disable_entity_check ? preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $needle) : $needle;
	...
	{
		if (!$this->disable_entity_check)
			$ent_arr = preg_split('~(&#\d{1,7};|&quot;|&amp;|&lt;|&gt;|&nbsp;|.)~u', preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $string), -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
		else
	...
	public function truncate($string, $length)
	{
		...
		if (!$this->disable_entity_check)
			$string = preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $string);
	 ...
	 * @return string
	 */
	public function shorten_text($string, $length = 384, $cutword = false, $ellipsis = '...', $exact = true, $buffer = 12)
	{
		// Does len include the ellipsis or are the ellipsis appended
		$ending = !empty($ellipsis) && $exact ? $this->strlen($ellipsis) : 0;
		...
		if ($this->strlen($string) > $length)
		{
			...
			if ($cutword)
			{
				$string = $this->substr($string, 0, $length - $ending);
				$space_pos = $this->strpos($string, ' ', 0, true);
				...
				if (!empty($space_pos) && ($length - $space_pos <= $buffer))
					$string = $this->substr($string, 0, $space_pos);

				$string = rtrim($string) . ($ellipsis ? $ellipsis : '');
			}
			else
				$string = $this->substr($string, 0, $length - $ending) . ($ellipsis ? $ellipsis : '');
		}
	 ...
	 */
	public function shorten_html($string, $length = 384, $ellipsis = '...', $exact = true)
	{
		// If its shorter than the maximum length, while accounting for html tags, simply return
		if ($this->strlen(preg_replace('~<.*?>~', '', $string)) <= $length)
			return $string;

		// Start off empty
		$total_length = $exact ? $this->strlen($ellipsis) : 0;
		$open_tags = array();
		$truncate = '';
		...
		foreach ($tags as $tag)
		{
			...
			$content_length = $this->strlen($tag[3]);
			...
			if ($content_length + $total_length > $length)
			{
				...
				$truncate .= $this->substr($tag[3], 0, $remaining);
				break;
			}
		...
		$space_pos = $this->strpos($truncate, ' ', 0, true);
		$space_pos = empty($space_pos) ? $length : $space_pos;
		$truncate_check = $this->substr($truncate, 0, $space_pos);

		// Make sure this would not cause a cut in the middle of a tag
		$lastOpenTag = (int) $this->strpos($truncate_check, '<', 0, true);
		$lastCloseTag = (int) $this->strpos($truncate_check, '>', 0, true);
		if ($lastOpenTag > $lastCloseTag)
		{
			...
			$space_pos = $this->strpos($truncate, $last_tag, 0, true) + strlen($last_tag);
			$space_pos = empty($space_pos) ? $length : $space_pos;
		}

		// Look at what we are going to cut off the end of our truncated string
		$bits = $this->substr($truncate, $space_pos);
		...
		// Cut it
		$truncate = $this->substr($truncate, 0, $space_pos);
	...
	public function strlen($string)
	{
		if (!$this->disable_entity_check)
		{
			...
			if (function_exists('mb_strlen'))
				...
				return strlen(preg_replace('~' . $ent_list . '|.~u', '_', preg_replace_callback($this->entity_check_reg, [$this, 'entity_fix'], $string)));
		}
		else
		{
	 ...
	 */
	public function htmlspecialchars__recursive($var, $level = 0)
	{
		if (!is_array($var))
			return $this->htmlspecialchars($var, ENT_QUOTES);
	 ...
	 */
	public function htmltrim__recursive($var, $level = 0)
	{
		// Remove spaces (32), tabs (9), returns (13, 10, and 11), nulls (0), and hard spaces. (160)
		if (!is_array($var))
			return $this->htmltrim($var);

		// Go through all the elements and remove the whitespace.
		foreach ($var as $k => $v)
			$var[$k] = $level > 25 ? null : $this->htmltrim__recursive($v, $level + 1);
