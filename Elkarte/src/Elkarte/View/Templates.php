...
class Templates
{
    ...
    protected function requireTemplate($template_name, $fatal)
    {
        ...
        elseif ($template_name != 'Errors' && $template_name != 'index' && $fatal)
            $GLOBALS['elk']['errors']->fatal_lang_error('theme_template_error', 'template', array((string) $template_name));
        elseif ($fatal)
            die($GLOBALS['elk']['errors']->log_error(sprintf(isset($txt['theme_template_error']) ? $txt['theme_template_error'] : 'Unable to load themes/default/%s.template.php!', (string) $template_name), 'template'));
        else
            return false;
    }
    ...
    public function loadSubTemplate($sub_template_name = 'main', $fatal = false)
    {
        ...
        if (function_exists($theme_function))
            $theme_function();
        elseif ($fatal === false)
            $GLOBALS['elk']['errors']->fatal_lang_error('theme_template_error', 'template', array((string) $sub_template_name));
        elseif ($fatal !== 'ignore')
            die($GLOBALS['elk']['errors']->log_error(sprintf(isset($txt['theme_template_error']) ? $txt['theme_template_error'] : 'Unable to load the %s sub template!', (string) $sub_template_name), 'template'));
    }
