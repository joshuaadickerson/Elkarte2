...
//		"/MSIE.*Windows XP/",	// misc comment spam
	...
	public function isBlackListed($agent, $uri)
	{
		return $this->checkUri($uri) || $this->checkUserAgent($agent);
	}

	public function checkUserAgent($agent)
	{
		foreach ($this->spambots_0 as $spambot) {
			$pos = strpos($agent, $spambot);
			if ($pos !== false && $pos == 0) {
				return true;
			}
		}

		foreach ($this->spambots as $spambot) {
			if (strpos($agent, $spambot) !== false) {
				return true;
			}
		}

		foreach ($this->spambots_regex as $spambot) {
			if (preg_match($spambot, $agent)) {
				return true;
			}
		}
	...
	{
		foreach ($this->spambots_url as $spambot) {
