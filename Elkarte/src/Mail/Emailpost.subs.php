...
function pbe_email_to_bbc($text, $html)
{
	...
	if ($html)
	{
		...
		$bbc_converter = new \Elkarte\Elkarte\Text\Html2BBC($text);
...
function pbe_prepare_text(&$message, &$subject = '', &$signature = '')
{
	...
	$subject = censor($GLOBALS['elk']['text']->un_htmlspecialchars($subject));
	...
	$mark_down = new \Elkarte\Elkarte\Text\Html2Md($message);
	...
	if ($signature !== '')
	{
		...
		$signature = trim($GLOBALS['elk']['text']->un_htmlspecialchars(strip_tags(strtr($signature, array('</tr>' => "   \n", '<br />' => "   \n", '</div>' => "\n", '</li>' => "   \n", '&#91;' => '[', '&#93;' => ']')))));
	}
...
function query_sender_wrapper($from)
{
	...
	{
		$bbc_wrapper = $GLOBALS['elk']['bbc'];
		$result['signature'] = trim($GLOBALS['elk']['text']->un_htmlspecialchars(strip_tags(strtr($bbc_wrapper->parseSignature($result['signature'], false), array('</tr>' => "   \n", '<br />' => "   \n", '</div>' => "\n", '</li>' => "   \n", '&#91;' => '[', '&#93;' => ']')))));
	}
