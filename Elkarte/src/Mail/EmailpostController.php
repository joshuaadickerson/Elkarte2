 ...
 * @package Maillist
 */
class EmailpostController extends AbstractController
{
	/**
	...
	public function action_pbe_post($data = null, $force = false, $key = null)
	{
		...
		$GLOBALS['elk']['hooks']->hook('mailist_checks_before', array($email_message, $pbe));
...
function pbe_create_topic($pbe, $email_message, $board_info)
{
	...
	$subject = strtr($GLOBALS['elk']['text']->htmlspecialchars($subject), array("\r" => '', "\n" => '', "\t" => ''));

	// Not to long not to short
	if ($GLOBALS['elk']['text']->strlen($subject) > 100)
		$subject = $GLOBALS['elk']['text']->substr($subject, 0, 100);
	elseif ($subject == '')
...
function pbe_load_text(&$html, $email_message, $pbe)
{
	if (!$html || ($html && preg_match_all('~<table.*?>~i', $email_message->body, $match) >= 2))
	{
		...
		$text = $GLOBALS['elk']['text']->un_htmlspecialchar($email_message->body);
	...
	$text = $GLOBALS['elk']['text']->htmltrim($text);
	if (empty($text))
		return;
