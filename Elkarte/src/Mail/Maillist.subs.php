...
function list_get_filter_parser($start, $items_per_page, $sort = '', $id = 0, $style = 'filter')
{
	...
	while ($row = $request->fetchAssoc())
	{
		$email_filters[$row['id_filter']] = array(
			...
			'filter_to' => '<strong>"</strong>' . $GLOBALS['elk']['text']->htmlspecialchars($row['filter_to']) . '<strong>"</strong>',
			'filter_from' => '<strong>"</strong>' . $GLOBALS['elk']['text']->htmlspecialchars($row['filter_from']) . '<strong>"</strong>',
			'filter_name' => $GLOBALS['elk']['text']->htmlspecialchars($row['filter_name']),
