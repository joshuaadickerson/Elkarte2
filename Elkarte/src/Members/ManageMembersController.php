 ...
 * @package Members
 */
class ManageMembersController extends AbstractController
{
	/**
	...
	public function action_index()
	{
		...
		require_once(SUBSDIR . '/Members/Members.subs.php');
		...
		if ($context['sub_action'] == 'query')
		{
			...
			foreach ($params as $param_name => $param_info)
			{
				...
				else
				{
					...
					$parameter = strtolower(strtr($GLOBALS['elk']['text']->htmlspecialchars($search_params[$param_name], ENT_QUOTES), array('%' => '\%', '_' => '\_', '*' => '%', '?' => '_')));
	 ...
	 */
	protected function _remindMember()
	{
		global $scripturl;
	 ...
	 */
	protected function _rejectMember()
	{
		deleteMembers($this->conditions['members']);
	 ...
	 */
	protected function _requireMember()
	{
		global $scripturl;
