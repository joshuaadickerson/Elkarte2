...
class MemberlistController extends AbstractController
{
	...
	public function action_index()
	{
		...
		$GLOBALS['elk']['hooks']->hook('memberlist_buttons');
	...
	public function action_mlall()
	{
		...
		if (!is_numeric($start))
		{
			if (preg_match('~^[^\'\\\\/]~u', $GLOBALS['elk']['text']->strtolower($start), $match) === 0)
		...
		if ($sort === 'real_name')
		{
			...
			{
				$this_letter = $GLOBALS['elk']['text']->strtolower($GLOBALS['elk']['text']->substr($context['members'][$i]['name'], 0, 1));

				if ($this_letter != $last_letter && preg_match('~[a-z]~', $this_letter) === 1)
				{
					$context['members'][$i]['sort_letter'] = $GLOBALS['elk']['text']->htmlspecialchars($this_letter);
	...
	public function action_mlsearch()
	{
			...
			|| isset($this->_req->post->search, $this->_req->post->fields))
		{
			...
			if ($GLOBALS['elk']['db']->db_case_sensitive())
			{
			...
			$query = $search == '' ? '= {string:blank_string}' : ($this->db()->db_case_sensitive() ? 'LIKE LOWER({string:search})' : 'LIKE {string:search}');
