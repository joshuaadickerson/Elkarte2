...
class DraftController extends AbstractController
{
	 ...
	 */
	protected function _query_limits($msgCount, $maxIndex)
	{
	 ...
	 */
	protected function _prepare_body_subject(&$body, &$subject, $id_draft, $default_subject, $smiley_enabled = true)
	{
		// Cleanup...
		if (empty($body))
			$body = '';

		$subject = $GLOBALS['elk']['text']->htmltrim($subject);
		if (empty($subject))
