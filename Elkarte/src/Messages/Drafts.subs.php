...
function create_pm_draft($draft, $recipientList)
{
	...
	// Return the new id
	return $result->insertId('{db_prefix}user_drafts', 'id_draft');
}
...
function saveDraft($draft, $check_last_save = false, $id_topic = 0)
{
	...
	if (!empty($check_last_save) && !empty($draft_info['poster_time']) && time() < $draft_info['poster_time'] + 5)
	{
		...
		{
			loadLanguage('Drafts');
			$GLOBALS['elk']['templates']->load('Xml');
	...
	if ($GLOBALS['elk']['text']->strlen($draft['subject']) > 100)
		$draft['subject'] = $GLOBALS['elk']['text']->substr($draft['subject'], 0, 100);
