...
class DraftsPersonalMessageModule implements ModuleInterface
{
	...
	public static function hooks(EventManager $eventsManager)
	{
		...
		if (!empty($modSettings['drafts_enabled']) && !empty($modSettings['drafts_post_enabled']))
		{
			...
			$GLOBALS['elk']['hooks']->add('integrate_sa_pm_index', 'Drafts_PersonalMessage_Module::integrate_sa_pm_index', '', false);
			$GLOBALS['elk']['hooks']->add('integrate_pm_areas', 'Drafts_PersonalMessage_Module::integrate_pm_areas', '', false);
	...
	{
		$subActions['showpmdrafts'] = array('controller' => 'DraftController', 'function' => 'action_showPMDrafts', 'permission' => 'pm_read');
	}
		...
		if ($context['drafts_pm_save'] && isset($_POST['save_draft']))
		{
			...
			if (!empty($context['drafts_pm_save']) && isset($_POST['id_pm_draft']))
			{
				...
				$draft = array(
					...
					'body' => $GLOBALS['elk']['text']->htmlspecialchars($_POST['message'], ENT_QUOTES, 'UTF-8', true),
					'subject' => strtr($GLOBALS['elk']['text']->htmlspecialchars($_POST['subject']), array("\r" => '', "\n" => '', "\t" => '')),
	...
	protected function _prepareDraftsContext($member_id, $id_pm = false)
	{
		...
		{
			$short_subject = empty($draft['subject']) ? $txt['drafts_none'] : $GLOBALS['elk']['text']->shorten_text(stripslashes($draft['subject']), self::$_subject_length);
			$context['drafts'][] = array(
