...
class Messages extends AbstractManager
{
	...
	function basicMessageInfo($id_msg, $override_permissions = false, $detailed = false)
	{
		...
		$messageInfo = $request->fetchAssoc();
		$request->free();
	...
	function checkMessagePermissions($message)
	{
		...
		if ($message['id_member'] == $user_info['id'] && !allowedTo('modify_any')) {
			...
			if ($message['approved'] && !empty($modSettings['edit_disable_time']) && $message['poster_time'] + ($modSettings['edit_disable_time'] + 5) * 60 < time())
				$this->errors->fatal_lang_error('modify_post_time_passed', false);
	 ...
	 * @todo this should be two methods: getAssociatedTopic() and associateTopic()
	 *
	...
	function canAccessMessage($id_msg, $check_approval = true)
	{
		...
		$message_info = $this->basicMessageInfo($id_msg);
	...
	{
		return $this->messagePointer($id_msg, $id_topic, false);
	}
	...
	{
		return $this->messagePointer($id_msg, $id_topic);
	}
	...
	function recordReport($message, $poster_comment)
	{
		...
		if ($request->numRows() != 0)
			list ($id_report, $ignore_all) = $request->fetchRow();
		$request->free();
		...
		if (!empty($id_report)) {
			...
			$id_report = $result->insertId('{db_prefix}log_reported', 'id_report');
		}
	 ...
	 * @return ResultInterface A request object
	 */
	...
	function loadMessageDetails($msg_selects, $msg_tables, $msg_parameters, $optional = array())
	{
		...
		$request = $this->loadMessageRequest($msg_selects, $msg_tables, $msg_parameters, $optional);
	...
	{
		$message_info = $this->basicMessageInfo((int)$messageID, true);
