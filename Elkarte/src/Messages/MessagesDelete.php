...
class MessagesDelete
{
	...
	public function removeMessage($message, $decreasePostCount = true, $check_permissions = true)
	{
		...
		if ($row['id_board'] != $this->_recycle_board && $row['icon'] != 'recycled')
		{
			...
			if ($request->numRows() == 0)
				$GLOBALS['elk']['errors']->fatal_lang_error('recycle_no_valid_board');
			list ($isRead, $last_board_msg) = $request->fetchRow();
			$request->free();
			...
			list ($id_recycle_topic, $first_topic_msg, $last_topic_msg) = $request->fetchRow();
			$request->free();
			...
			$topicID = empty($id_recycle_topic) ? $result->insertId('{db_prefix}topics', 'id_topic') : $id_recycle_topic;
			...
			if ($topicID > 0)
			{
				...
				require_once('Messages.subs.php');
		...
		if (!$recycle)
		{
			...
			$GLOBALS['elk']['hooks']->hook('remove_message', array($message));
		}
			...
			if (!in_array(0, $delete_any) && !in_array($row['id_board'], $delete_any))
			{
				...
				if ($row['id_member'] == $user_info['id'])
				{
					if (!$delete_own)
					{
						...
						{
							if (!$delete_replies)
								$GLOBALS['elk']['errors']->fatal_lang_error('cannot_delete_replies', 'permission');
						}
						else
							$GLOBALS['elk']['errors']->fatal_lang_error('cannot_delete_own', 'permission');
					}
					elseif (($row['id_member_poster'] != $user_info['id'] || !$delete_replies) && !empty($modSettings['edit_disable_time']) && $row['poster_time'] + $modSettings['edit_disable_time'] * 60 < time())
						$GLOBALS['elk']['errors']->fatal_lang_error('modify_post_time_passed', false);
				}
				elseif ($row['id_member_poster'] == $user_info['id'])
				{
					...
					$GLOBALS['elk']['errors']->fatal_lang_error('cannot_delete_any', 'permission');
			}
		...
		if ($row['id_first_msg'] == $row['id_msg'])
		{
			if (empty($board) || $row['id_board'] != $board)
			{
				...
				if ($row['id_member'] != $user_info['id'] && !$remove_any)
					$GLOBALS['elk']['errors']->fatal_lang_error('cannot_remove_any', 'permission');
				elseif (!$remove_any && !$remove_own)
					$GLOBALS['elk']['errors']->fatal_lang_error('cannot_remove_own', 'permission');
			}
			else
			{
			...
			if (!empty($row['num_replies']))
				$GLOBALS['elk']['errors']->fatal_lang_error('delFirstPost', false);
	 ...
	 */
	protected function _mergePosts($msgs, $from_topic, $target_topic)
	{
		$db = $GLOBALS['elk']['db'];
		...
		list ($from_board, $from_first_msg, $from_replies, $from_unapproved_posts) = $request->fetchRow();
		$request->free();
		...
		list ($target_board, $target_first_msg, $target_replies, $target_unapproved_posts, $count_posts) = $request->fetchRow();
		$request->free();
