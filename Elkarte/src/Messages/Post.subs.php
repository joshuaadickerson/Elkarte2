...
function preparsecode_font_callback($matches)
{
	...
	$font = trim($GLOBALS['elk']['text']->un_htmlspecialchars($fonts[0]), ' "\'');
...
function un_preparsecode($message)
{
	...
	for ($i = 0, $n = count($parts); $i < $n; $i++)
	{
		$GLOBALS['elk']['hooks']->hook('unpreparse_code', array(&$message, &$parts, &$i));
	}
...
function fixTags(&$message)
{
	...
	$GLOBALS['elk']['hooks']->hook('fixtags', array(&$fixArray, &$message));
...
function resizeBBCImages(&$message)
{
	...
	foreach ($matches[0] as $match => $dummy)
	{
		...
		{
			list ($width, $height) = url_image_size($GLOBALS['elk']['text']->un_htmlspecialchars($matches[4][$match]));
...
function createPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	$GLOBALS['elk']['hooks']->hook('before_create_post', array(&$msgOptions, &$topicOptions, &$posterOptions, &$message_columns, &$message_parameters));
	...
	$msgOptions['id'] = $result->insertId('{db_prefix}messages', 'id_msg');
	...
	$GLOBALS['elk']['hooks']->hook('create_post', array($msgOptions, $topicOptions, $posterOptions, $message_columns, $message_parameters));
	...
	if ($new_topic)
	{
		...
		$GLOBALS['elk']['hooks']->hook('before_create_topic', array(&$msgOptions, &$topicOptions, &$posterOptions, &$topic_columns, &$topic_parameters));
		...
		$topicOptions['id'] = $result->insertId('{db_prefix}topics', 'id_topic');
		...
		$GLOBALS['elk']['hooks']->hook('create_topic', array($msgOptions, $topicOptions, $posterOptions));
	}
	...
	else
	{
		...
		$GLOBALS['elk']['hooks']->hook('before_modify_topic', array(&$topics_columns, &$update_parameters, &$msgOptions, &$topicOptions, &$posterOptions));
...
function modifyPost(&$msgOptions, &$topicOptions, &$posterOptions)
{
	...
	$GLOBALS['elk']['hooks']->hook('before_modify_post', array(&$messages_columns, &$update_parameters, &$msgOptions, &$topicOptions, &$posterOptions, &$messageInts));
...
function updateLastMessages($setboards, $id_msg = 0)
{
	...
	foreach ($setboards as $id_board)
	{
		...
		if (!empty($board) && $id_board == $board)
			$parents = $board_info->parentBoards();
		else
			$parents = $GLOBALS['elk']['boards.manager']->getBoardParents($id_board);
...
function getFormMsgSubject($editing, $topic, $first_subject = '', $msg_id = 0)
{
	...
	switch ($editing)
	{
		...
		case 2:
		{
			...
			if ($request->numRows() == 0)
				$GLOBALS['elk']['errors']->fatal_lang_error('quoted_post_deleted', false);
			list ($form_subject, $mname, $mdate, $form_message) = $request->fetchRow();
			$request->free();

			// Add 'Re: ' to the front of the quoted subject.
			$response_prefix = response_prefix();
			if (trim($response_prefix) != '' && $GLOBALS['elk']['text']->strpos($form_subject, trim($response_prefix)) !== 0)
		...
		case 3:
		{
			...
			$response_prefix = response_prefix();
			if (trim($response_prefix) != '' && $form_subject != '' && $GLOBALS['elk']['text']->strpos($form_subject, trim($response_prefix)) !== 0)
				$form_subject = $response_prefix . $form_subject;
