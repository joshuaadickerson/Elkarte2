...
class PostController extends AbstractController
{
	...
	public function action_post()
	{
		...
		if (isset($_REQUEST['message']) || $this->_post_errors->hasErrors())
		{
			...
			$form_subject = strtr($GLOBALS['elk']['text']->htmlspecialchars($_REQUEST['subject']), array("\r" => '', "\n" => '', "\t" => ''));
			$form_message = $GLOBALS['elk']['text']->htmlspecialchars($_REQUEST['message'], ENT_QUOTES, 'UTF-8', true);

			// Make sure the subject isn't too long - taking into account special characters.
			if ($GLOBALS['elk']['text']->strlen($form_subject) > 100)
				$form_subject = $GLOBALS['elk']['text']->substr($form_subject, 0, 100);

			// Are you... a guest?
			if ($user_info['is_guest'])
			{
				$context['name'] = !isset($_REQUEST['guestname']) ? '' : $GLOBALS['elk']['text']->htmlspecialchars(trim($_REQUEST['guestname']));
				$context['email'] = !isset($_REQUEST['email']) ? '' : $GLOBALS['elk']['text']->htmlspecialchars(trim($_REQUEST['email']));
			...
			if ($really_previewing === true)
			{
				...
				elseif (!empty($modSettings['max_messageLength']) && $GLOBALS['elk']['text']->strlen($form_message) > $modSettings['max_messageLength'])
	...
	public function action_post2()
	{
		...
		if ($posterIsGuest)
		{
			$_POST['guestname'] = !isset($_POST['guestname']) ? '' : $GLOBALS['elk']['text']->htmlspecialchars(trim($_POST['guestname']));
			$_POST['email'] = !isset($_POST['email']) ? '' : $GLOBALS['elk']['text']->htmlspecialchars(trim($_POST['email']));

			if ($_POST['guestname'] == '' || $_POST['guestname'] == '_')
				$this->_post_errors->addError('no_name');

			if ($GLOBALS['elk']['text']->strlen($_POST['guestname']) > 25)
		...
		if (!isset($_POST['subject']) || $GLOBALS['elk']['text']->htmltrim($GLOBALS['elk']['text']->htmlspecialchars($_POST['subject'])) === '')
			$this->_post_errors->addError('no_subject');

		if (!isset($_POST['message']) || $GLOBALS['elk']['text']->htmltrim($GLOBALS['elk']['text']->htmlspecialchars($_POST['message'], ENT_QUOTES)) === '')
			$this->_post_errors->addError('no_message');
		elseif (!empty($modSettings['max_messageLength']) && $GLOBALS['elk']['text']->strlen($_POST['message']) > $modSettings['max_messageLength'])
			$this->_post_errors->addError(array('long_message', array($modSettings['max_messageLength'])));
		else
		{
			// Prepare the message a bit for some additional testing.
			$_POST['message'] = $GLOBALS['elk']['text']->htmlspecialchars($_POST['message'], ENT_QUOTES, 'UTF-8', true);
			...
			if ($GLOBALS['elk']['text']->htmltrim(strip_tags($bbc_parser->parseMessage($_POST['message'], false), '<img>')) === '' && (!allowedTo('admin_forum') || strpos($_POST['message'], '[html]') === false))
				$this->_post_errors->addError('no_message');
		}
		...
		$_POST['subject'] = strtr($GLOBALS['elk']['text']->htmlspecialchars($_POST['subject']), array("\r" => '', "\n" => '', "\t" => ''));
		...
		if ($GLOBALS['elk']['text']->strlen($_POST['subject']) > 100)
			$_POST['subject'] = $GLOBALS['elk']['text']->substr($_POST['subject'], 0, 100);
		...
		if ($board_info['num_topics'] == 0)
			$GLOBALS['elk']['cache']->remove('board-' . $board);
	...
	public function action_quotefast()
	{
		...
		if (!empty($can_view_post))
		{
			...
			$context['quote']['text'] = strtr($GLOBALS['elk']['text']->un_htmlspecialchars($context['quote']['xml']), array('\'' => '\\\'', '\\' => '\\\\', "\n" => '\\n', '</script>' => '</\' + \'script>'));
			$context['quote']['xml'] = strtr($context['quote']['xml'], array('&nbsp;' => '&#160;', '<' => '&lt;', '>' => '&gt;'));

			$context['quote']['mozilla'] = strtr($GLOBALS['elk']['text']->htmlspecialchars($context['quote']['text']), array('&quot;' => '"'));
		}
	...
	public function action_jsmodify()
	{
		...
		{
			if ($GLOBALS['elk']['text']->htmltrim($GLOBALS['elk']['text']->htmlspecialchars($_POST['message'])) === '')
			{
				$this->_post_errors->addError('no_message');
				unset($_POST['message']);
			}
			...
			else
			{
				...
				if ($GLOBALS['elk']['text']->htmltrim(strip_tags($bbc_parser->parseMessage($_POST['message'], false), '<img>')) === '')
				{
					$this->_post_errors->addError('no_message');
					unset($_POST['message']);
				}
