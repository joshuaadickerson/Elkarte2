 ...
 * @package News
 */
class ManageNewsController extends AbstractController
{
	/**
	 * News settings form.
	 * @var SettingsForm
			...
			foreach ($toClean as $key => $type)
			{
				...
				foreach ($temp as $index => $member)
				{
					if (strlen(trim($member)) > 0)
						$temp[$index] = $GLOBALS['elk']['text']->htmlspecialchars($GLOBALS['elk']['text']->strtolower(trim($member)));
					else
						unset($temp[$index]);
				}
	...
	protected function _settings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_news_settings', array(&$config_vars));
