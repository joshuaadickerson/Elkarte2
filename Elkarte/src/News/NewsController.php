...
class NewsController extends AbstractController
{
	...
	public function action_showfeed()
	{
		...
		$context['feed_title'] = encode_special(strip_tags($GLOBALS['elk']['text']->un_htmlspecialchars($context['forum_name']) . (isset($feed_title) ? $feed_title : '')));
	...
	public function action_xmlnews($xml_format)
	{
		...
		foreach ($results as $row)
		{
			...
			if (!empty($modSettings['xmlnews_maxlen']) && $GLOBALS['elk']['text']->strlen(str_replace('<br />', "\n", $row['body'])) > $modSettings['xmlnews_maxlen'])
				$row['body'] = strtr($GLOBALS['elk']['text']->shorten_text(str_replace('<br />', "\n", $row['body']), $modSettings['xmlnews_maxlen'], true), array("\n" => '<br />'));
			...
			if ($xml_format === 'rss' || $xml_format === 'rss2')
			{
				$data[] = array(
					...
					'description' => cdata_parse(strtr($GLOBALS['elk']['text']->un_htmlspecialchars($row['body']), '&', '&#x26;')),
...
function fix_possible_url($val)
{
	...
	$GLOBALS['elk']['hooks']->hook('fix_url', array(&$val));
...
function cdata_parse($data, $ns = '')
{
	...
	for ($pos = 0, $n = $GLOBALS['elk']['text']->strlen($data); $pos < $n; null)
	{
		$positions = array(
			$GLOBALS['elk']['text']->strpos($data, '&', $pos),
			$GLOBALS['elk']['text']->strpos($data, ']', $pos),
		);

		if ($ns != '')
			$positions[] = $GLOBALS['elk']['text']->strpos($data, '<', $pos);
		...
		if ($pos - $old > 0)
			$cdata .= $GLOBALS['elk']['text']->substr($data, $old, $pos - $old);

		if ($pos >= $n)
			break;

		if ($GLOBALS['elk']['text']->substr($data, $pos, 1) === '<')
		{
			$pos2 = $GLOBALS['elk']['text']->strpos($data, '>', $pos);
			if ($pos2 === false)
				$pos2 = $n;

			if ($GLOBALS['elk']['text']->substr($data, $pos + 1, 1) === '/')
				$cdata .= ']]></' . $ns . ':' . $GLOBALS['elk']['text']->substr($data, $pos + 2, $pos2 - $pos - 1) . '<![CDATA[';
			else
				$cdata .= ']]><' . $ns . ':' . $GLOBALS['elk']['text']->substr($data, $pos + 1, $pos2 - $pos) . '<![CDATA[';

			$pos = $pos2 + 1;
		}
		elseif ($GLOBALS['elk']['text']->substr($data, $pos, 1) === ']')
		{
			$cdata .= ']]>&#093;<![CDATA[';
			$pos++;
		}
		elseif ($GLOBALS['elk']['text']->substr($data, $pos, 1) === '&')
		{
			...
			$ent = $GLOBALS['elk']['text']->substr($data, $pos + 1, $pos2 - $pos - 1);

			if ($GLOBALS['elk']['text']->substr($data, $pos + 1, 1) === '#')
				$cdata .= ']]>' . $GLOBALS['elk']['text']->substr($data, $pos, $pos2 - $pos + 1) . '<![CDATA[';
