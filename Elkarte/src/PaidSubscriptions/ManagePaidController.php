 ...
 */
class ManagePaidController extends AbstractController
{
	/**
	...
	protected function _settings()
	{
		...
		$GLOBALS['elk']['hooks']->hook('modify_subscription_settings', array(&$config_vars));
	...
	public function action_view()
	{
		...
		if (empty($modSettings['paid_currency_symbol']))
			$GLOBALS['elk']['errors']->fatal_lang_error('paid_not_set_currency', false, array($scripturl . '?action=Admin;area=paidsubscribe;sa=settings'));
	...
	public function action_modify()
	{
		...
		if (isset($this->_req->post->delete_confirm) && isset($this->_req->query->delete))
		{
			...
			$GLOBALS['elk']['hooks']->hook('delete_subscription', array($context['sub_id']));
		...
		elseif (isset($this->_req->post->save_sub))
		{
			...
			if (empty($context['log_id']))
			{
				...
				if (alreadySubscribed($context['sub_id'], $member['id_member']))
					$GLOBALS['elk']['errors']->fatal_lang_error('member_already_subscribed');
