...
class PaidSubscriptions extends AbstractManager
{
	/**
	...
	function list_getSubscribedUserCount($id_sub, $search_string, $search_vars = array())
	{
		...
		list ($memberCount) = $request->fetchRow();
		$request->free();
	...
	function addSubscription($id_subscribe, $id_member, $renewal = '', $forceStartTime = 0, $forceEndTime = 0)
	{
		...
		if ($request->numRows() != 0) {
			list ($id_sublog, $endtime, $starttime) = $request->fetchRow();
		...
		try {
			...
			foreach ($files as $file) {
				if ($file->isDir()) {
					...
					$namespace = 'Elkarte\\Subscriptions\\' . $dirname;

					// Let the autoloader do the work
					$payment_class = $namespace . "\\Payment";
					$display_class = $namespace . "\\Display";

					//require_once($file->getPathname());

					$gateways[] = array(
						'filename' => $dirname,
						'code' => strtolower($dirname),
	...
	function insertSubscription($insert)
	{
		...
		return $result->insertId('{db_prefix}subscriptions', 'id_subscribe');
	}
	...
	function countActiveSubscriptions($sub_id)
	{
		...
		list ($isActive) = $request->fetchRow();
		$request->free();
	...
	function getSubscriptionStatus($log_id)
	{
		...
		if ($request->numRows() !== 0)
			list ($status['id_member'], $status['old_status']) = $request->fetchRow();
		$request->free();
