...
class PollPostModule implements ModuleInterface
{
	...
	public function finalize_post_form(&$destination, &$page_title, $template_layers)
	{
		...
		if (self::$_make_poll)
		{
			...
			$GLOBALS['elk']['templates']->load('Poll');
	...
	protected function _createPoll($options, $user_name)
	{
		...
		if (!empty($poll_expire) && $poll_expire < 1)
			...
			$GLOBALS['elk']['errors']->fatal_lang_error('poll_range_error', false);
		...
		$poll_options = $GLOBALS['elk']['text']->htmlspecialchars__recursive($options['options']);
