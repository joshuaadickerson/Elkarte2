...
class Profile extends AbstractManager
{
	public function __construct(DatabaseInterface $db, Cache $cache, Hooks $hooks, Errors $errors, StringUtil $text, MembersManager $members_manager)
	{
		$this->db = $db;
		$this->cache = $cache;
		$this->errors = $errors;
		$this->hooks = $hooks;
		$this->text = $text;
		$this->members_manager = $members_manager;
	}
	...
	function currentMemberID($fatal = true, $reload_id = false)
	{
		...
		if (isset($_REQUEST['user']))
			$memberResult = $this->members_manager->loadMemberData($_REQUEST['user'], true, 'profile');
		// ... or by id_member?
		elseif (!empty($_REQUEST['u']))
			$memberResult = $this->members_manager->loadMemberData((int)$_REQUEST['u'], false, 'profile');
		// If it was just ?action=profile, edit your own profile.
		else
			$memberResult = $this->members_manager->loadMemberData($user_info['id'], false, 'profile');
		...
		if (!is_array($memberResult)) {
			...
			if ($fatal)
				$GLOBALS['elk']['errors']->fatal_lang_error('not_a_user', false);
			else
				return false;
		}
	...
	function loadCustomFields($memID, $area = 'summary', array $custom_fields = array())
	{
		...
		$GLOBALS['elk']['hooks']->hook('load_custom_profile_fields', array($memID, $area));
	}
	...
	function loadProfileFields($force_reload = false)
	{
		...
		$GLOBALS['elk']['hooks']->hook('load_profile_fields', array(&$profile_fields));
	...
	function saveProfileChanges(&$profile_vars, $memID)
	{
		...
		$GLOBALS['elk']['hooks']->hook('save_profile_changes', array(&$profile_bools, &$profile_ints, &$profile_floats, &$profile_strings));
	...
	function makeThemeChanges($memID, $id_theme)
	{
		...
		if (empty($context['password_auth_failed'])) {
			...
			foreach ($themes as $t)
				$GLOBALS['elk']['cache']->remove('theme_settings-' . $t . ':' . $memID);
		}
	}
	...
	function makeCustomFieldChanges($memID, $area, $sanitize = true)
	{
		...
		$GLOBALS['elk']['hooks']->hook('save_custom_profile_fields', array(&$changes, &$log_changes, $memID, $area, $sanitize));
	...
	function profileSendActivation()
	{
		...
		$GLOBALS['elk']['templates']->load('Register');
	...
	function profileSaveGroups(&$value)
	{
		...
		if (in_array(1, explode(',', $old_profile['additional_groups'])) || $old_profile['id_group'] == 1) {
			...
			if (!$stillAdmin) {
				...
				if (empty($another))
					$GLOBALS['elk']['errors']->fatal_lang_error('at_least_one_admin', 'critical');
			}
		}
	...
	function list_getUserWarningCount($memID)
	{
		...
		list ($total_warnings) = $request->fetchRow();
		$request->free();
	...
	function getNumAttachments($boardsAllowed, $memID)
	{
		...
		list ($attachCount) = $request->fetchRow();
		$request->free();
	...
	function getNumUnwatchedBy($memID)
	{
		...
		list ($unwatchedCount) = $request->fetchRow();
		$request->free();
	...
	function count_user_posts($memID, $board = null)
	{
		...
		list ($msgCount) = $request->fetchRow();
		$request->free();
	...
	function load_user_posts($memID, $start, $count, $range_limit = '', $reverse = false, $board = null)
	{
		...
		for ($i = 0; $i < 2; $i++) {
			...
			if ($request->numRows() === $count || empty($range_limit))
				break;
			else
				$range_limit = '';
		}
	...
	function getMembersIPs($memID)
	{
		...
		if ($modSettings['totalMessages'] > 50000 && $user_profile[$memID]['posts'] > 500) {
			...
			list ($max_msg_member) = $request->fetchRow();
			$request->free();
