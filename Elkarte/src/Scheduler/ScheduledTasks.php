...
class ScheduledTasks
{
	...
	function run_this_task($id_task, $task_name)
	{
		...
		Elk_Autoloader::getInstance()->register(SUBSDIR . '/ScheduledTasks', '\\ElkArte\\Sources\\subs\\ScheduledTasks');

		// Let's start logging the task and saying we failed it
		$log_task_id = logTask(0, $id_task);

		$class = '\\ElkArte\\Sources\\subs\\ScheduledTasks\\' . implode('_', array_map('ucfirst', explode('_', $task_name)));
	...
	function removeScheduleTaskImmediate($task, $calculateNextTrigger = true)
	{
		...
		if (isset($scheduleTaskImmediate[$task])) {
			...
			if ($calculateNextTrigger) {
				$this->calculateNextTrigger($task);
			}
		}
	}
}