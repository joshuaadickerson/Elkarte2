		...
		while ($row = $request->fetchAssoc())
		{
			...
			elseif ($row['note_type'] === 'topic')
			{
				if ($maillist)
				{
					...
					$row['body'] = $GLOBALS['elk']['text']->shorten_text($row['body'], !empty($modSettings['digest_preview_length']) ? $modSettings['digest_preview_length'] : 375, true);
		...
		if ($maillist)
		{
			foreach ($types['reply'] as $id => $board)
			{
				foreach ($board['lines'] as $topic)
				{
					...
					$body = $GLOBALS['elk']['text']->shorten_text($body, !empty($modSettings['digest_preview_length']) ? $modSettings['digest_preview_length'] : 375, true);
					$body = preg_replace("~\n~s", "\n  ", $body);
