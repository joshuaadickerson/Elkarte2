...
class Fulltext_Search extends SearchAPI
{
	...
	protected function _getMinWordLength()
	{
		...
		{
			list (, $min_word_length) = $request->fetchRow();
			$request->free();
		}
	...
	{
		$x = $GLOBALS['elk']['text']->strlen($a) - (in_array($a, $this->_excludedWords) ? 1000 : 0);
		$y = $GLOBALS['elk']['text']->strlen($b) - (in_array($b, $this->_excludedWords) ? 1000 : 0);
	...
	public function prepareIndexes($word, &$wordsSearch, &$wordsExclude, $isExcluded)
	{
		...
		if (empty($modSettings['search_force_index']))
		{
			...
			if (count($subwords) > 1 && preg_match('~[.:@$]~', $word))
			{
				...
				if (($GLOBALS['elk']['text']->strlen(current($subwords)) < $this->min_word_length) && ($GLOBALS['elk']['text']->strlen(next($subwords)) < $this->min_word_length))
				{
					$wordsSearch['words'][] = trim($word, '/*- ');
					$wordsSearch['complex_words'][] = count($subwords) === 1 ? $word : '"' . $word . '"';
				}
			}
			elseif ($GLOBALS['elk']['text']->strlen(trim($word, '/*- ')) < $this->min_word_length)
			{
