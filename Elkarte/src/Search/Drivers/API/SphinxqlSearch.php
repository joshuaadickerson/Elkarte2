...
class Sphinxql_Search extends SearchAPI
{
	...
	public function searchQuery($search_params, $search_words, $excluded_words, &$participants, &$search_results)
	{
		...
		$cached_results = null;
		if (!$GLOBALS['elk']['cache']->getVar($cached_results, 'ssearchql_results_' . md5($user_info['query_see_board'] . '_' . $context['params'])))
		{
			...
			if ($request === false)
			{
				...
				if (mysqli_error($mySphinx))
					$GLOBALS['elk']['errors']->log_error(mysqli_error($mySphinx));
			...
			$GLOBALS['elk']['cache']->put('searchql_results_' . md5($user_info['query_see_board'] . '_' . $context['params']), $cached_results, 600);
		}
	 ...
	 */
	protected function _constructQuery($string)
	{
	...
	protected function _cleanString($string)
	{
		...
		// Lowercase string
		$string = $GLOBALS['elk']['text']->strtolower($string);
