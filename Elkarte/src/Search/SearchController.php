...
class SearchController extends AbstractController
{
	...
	public function action_search()
	{
		...
		if (isset($_REQUEST['search']))
			$context['search_params']['search'] = $GLOBALS['elk']['text']->un_htmlspecialchar($_REQUEST['search']);
	...
	public function action_results()
	{
		...
		// Too long?
		elseif ($GLOBALS['elk']['text']->strlen($this->_search->param('search')) > $context['search_string_limit'])
		...
		// *** A last error check
		$GLOBALS['elk']['hooks']->hook('search_errors');
		...
		if (!empty($context['topics']))
		{
			...
			$GLOBALS['elk']['hooks']->hook('search_message_list', array(&$msg_list, &$posters));
		...
		$GLOBALS['elk']['cache']->remove('search_start:' . ($user_info['is_guest'] ? $user_info['ip'] : $user_info['id']));
		...
		$context['jump_to'] = array(
			...
			'board_name' => addslashes($GLOBALS['elk']['text']->un_htmlspecialchars($txt['select_destination'])),
		);
	}
	...
	public function prepareSearchContext_callback($reset = false)
	{
		...
		if ($context['compact'])
		{
			...
			if ($GLOBALS['elk']['text']->strlen($message['body']) > $charLimit)
			{
				if (empty($context['key_words']))
					$message['body'] = $GLOBALS['elk']['text']->substr($message['body'], 0, $charLimit) . '<strong>...</strong>';
				else
				{
					$matchString = '';
					...
					{
						$keyword = $GLOBALS['elk']['text']->un_htmlspecialchar($keyword);
					...
					$matchString = $GLOBALS['elk']['text']->un_htmlspecialchar(substr($matchString, 0, -1));

					$message['body'] = $GLOBALS['elk']['text']->un_htmlspecialchar(strtr($message['body'], array('&nbsp;' => ' ', '<br />' => "\n", '&#91;' => '[', '&#93;' => ']', '&#58;' => ':', '&#64;' => '@')));
		...
		if (!empty($options['display_quick_mod']))
		{
			...
			$GLOBALS['elk']['hooks']->hook('quick_mod_actions_search');
		}
		...
		$GLOBALS['elk']['hooks']->hook('search_message_context', array($counter, &$output));
	 ...
	 */
	protected function _setup_weight_factors()
	{
		...
		$GLOBALS['elk']['hooks']->hook('search_weights', array(&$this->_weight_factors));
	 ...
	 * @param int[] $weights
	 */
	protected function _calculate_weights($factors, $weights)
	{
		$this->_weight = array();
	 ...
	 * @param mixed[] $array
	 */
	protected function _fill_default_search_params($array)
	{
	 ...
	 */
	protected function _increase_pointer()
	{
