...
class Spiders extends AbstractManager
{
	/**
	...
	function spiderCheck()
	{
		...
		if (!$GLOBALS['elk']['cache']->getVar($spider_data, 'spider_search', 300)) {
			$request = $db->query('', '
			...
			$GLOBALS['elk']['cache']->put('spider_search', $spider_data, 300);
		}
	...
	function getNumSpiders()
	{
		...
		list ($numSpiders) = $request->fetchRow();
		$request->free();
	...
	function getNumSpiderStats($time = null)
	{
		...
		list ($numStats) = $request->fetchRow();
		$request->free();
	...
	function spidersStatsDates()
	{
		...
		list ($min_date, $max_date) = $request->fetchRow();
		$request->free();
