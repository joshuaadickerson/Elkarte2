...
class MoveTopicController extends AbstractController
{
	...
	public function action_movetopic()
	{
		...
		if (empty($context['categories']) || $context['num_boards'] == 1)
			$this->_errors->fatal_lang_error('moveto_noboards', false);
		...
		$this->_templates->load('MoveTopic');
		$this->_prep_template();
	}
	 ...
	 */
	protected function _prep_template()
	{
	...
	protected function _check_access()
	{
		...
		if (empty($this->_topic_info))
			$this->_errors->fatal_lang_error('topic_gone', false);
		...
		if (!allowedTo('move_any') && ($this->_topic_info['id_member_started'] == $user_info['id'] && !allowedTo('move_own')))
			$this->_errors->fatal_lang_error('cannot_move_any', false);
	}
	 ...
	 * @return bool
	 */
	protected function _check_access_2()
	{
		global $user_info;
	 ...
	 */
	protected function _rename_topic()
	{
		global $context;
		...
		if (isset($this->_req->post->reset_subject, $this->_req->post->custom_subject) && $this->_req->post->custom_subject != '')
		{
			...
			if ($GLOBALS['elk']['text']->strlen($custom_subject) > 100)
				$custom_subject = $GLOBALS['elk']['text']->substr($custom_subject, 0, 100);
	 ...
	 */
	protected function _post_redirect()
	{
	 ...
	 */
	protected function _count_update()
	{
		global $board;
