...
class Unread
{
	...
	/**
	 * @var \Elkarte\Database\Drivers\DatabaseInterface
	 */
	private $_db;
	...
	protected function _getUnreadTopics($join, $start, $limit, $include_avatars = false)
	{
		...
		$topic_util = new \Elkarte\Topics\TopicUtil;
	...
	protected function _countUnreadReplies()
	{
		if (!empty($this->_have_temp_table))
		{
			...
			list ($this->_num_topics) = $request->fetchRow();
			$request->free();
			$this->_min_message = 0;
		}
		else
		{
			...
			list ($this->_num_topics, $this->_min_message) = $request->fetchRow();
			$request->free();
		}
	}
